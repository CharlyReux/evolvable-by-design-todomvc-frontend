...
>[!WARNING] 
>In order to simplify the usage of pivo, you can replace the content of the todoService with the content in src/commons/TodoServicePivo.md, which adds type hinting, and guides you to implement the fetch and the create method.

### First implementation
1. Modify the TodoList component to use semantic data(You can use the utility component `with-semantic-data-required.jsx`, and get inspiration from the [tutorial](https://github.com/CharlyReux/evolvable-by-design-tutorial/tree/main#setting-up-pivo-in-our-application))
2. Implement the constructor of the todoService.
3. Instantiate the service in the TodolistPage(You will have to modify The todoService and the TodoList component, you ), note that the openAPI specification is available at /rest with the OPTIONS HTTP verb.
4. Implement the methods in the TodoService (you will have to modify the switchTodoStatus method as well)
5. Once your app works and is running, run the following command: `./experiment -n`, and you will be assigned with an update to implement.
